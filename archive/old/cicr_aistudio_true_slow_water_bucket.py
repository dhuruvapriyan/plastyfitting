#!/usr/bin/env python3
"""
Primed-Junctional CICR Model V4.3 — True Slow Water Bucket
...
3. k_fill is strictly bounded so it takes dozens of spikes (seconds to mins) to fill, 
   perfectly matching Hong & Ross (2007).
...
def _apply_primed_junctional_debug(cai_1syn, t, cp, cq, is_apical, dp):
    ...
    for i in range(n - 1):
        ...
        # No effcai gating required. The spike width natively limits the fill amount.
...
class PrimedJunctionalCICRModel(CICRModel):
    DESCRIPTION = "Primed-Junctional V4.3 (True Slow Water Bucket)"

    FIT_PARAMS = [
        ("gamma_d_GB_GluSynapse", 50.0, 250.0),      
        ("gamma_p_GB_GluSynapse", 50.0, 300.0),      
        ("a00", 1.0, 2.5),             # LOWERED MAX: Forces theta_d to be reachable
        ("a01", 1.0, 5.0),             
        ("a10", 1.0, 5.0),
        ("a11", 1.0, 5.0),
        ("a20", 1.0, 2.5),             # LOWERED MAX: Forces theta_d to be reachable
        ("a21", 1.0, 5.0),
        ("a30", 1.0, 10.0),
        ("a31", 1.0, 5.0),
        ("k_fill", 0.001, 2.0),        
        ("tau_leak", 5000.0, 100000.0), # LOWERED MAX: Ensures ER resets between bursts
        ("K_clip", 0.0001, 0.001),     
        ("K_trig", 0.001, 0.005),      
        ("n_trig", 2.0, 15.0),         
        ("Vmax_CICR", 0.001, 5.0),     
...
# {
#     "gamma_d_GB_GluSynapse": 151.91582635632366,
#     "gamma_p_GB_GluSynapse": 151.16769072194217,
#     "a00": 1.0082606258438696,
#     "a01": 1.592542825674953,
#     "a10": 1.1202262451746339,
#     "a11": 2.866226573024486,
#     "a20": 1.8691619208033217,
#     "a21": 3.2775124204626835,
#     "a30": 2.0184488801739953,
#     "a31": 3.0953968247762824,
#     "k_fill": 1.9652416722619974,
#     "tau_leak": 210708.31874287373,
#     "K_clip": 0.0005050025079862597,
#     "K_trig": 0.001415502191228662,
#     "n_trig": 5.851991204364424,
#     "Vmax_CICR": 4.478195956390422,
#     "tau_CICR": 772.3213106652026,
#     "g_cicr": 44797.46514907753,
#     "tau_eff": 464.31609425364866
# }

    DEFAULT_PARAMS = {
        "gamma_d_GB_GluSynapse": 151.91582635632366, "gamma_p_GB_GluSynapse": 151.16769072194217,
        "a00": 1.0082606258438696, "a01": 1.592542825674953, "a10": 1.1202262451746339, "a11": 2.866226573024486,
        "a20": 1.8691619208033217, "a21": 3.2775124204626835, "a30": 2.0184488801739953, "a31": 3.0953968247762824,
        "k_fill": 1.9652416722619974, "tau_leak": 210708.31874287373, 
        "K_clip": 0.0005050025079862597, "K_trig": 0.001415502191228662, "n_trig": 5.851991204364424,
        "Vmax_CICR": 4.478195956390422, "tau_CICR": 772.3213106652026,
        "g_cicr": 44797.46514907753, "tau_eff": 464.31609425364866,
    }
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            def scan_step(carry, inputs):
                ...
                # 2. True Slow Water Bucket (No effcai gate)
