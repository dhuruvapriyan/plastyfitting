...
8) Long-term synaptic plasticity. Calcium-based STDP model based on Graupner and
   Brunel 2012.
9) [CICR+] ER store dynamics with Li-Rinzel IP3R gating. When enable_CICR=1,
   mGluR-driven IP3 production drives a Li-Rinzel IP3R (m^3*n^3*h^3) with
   SERCA (Hill n=2) and passive leak governing ER Ca2+ (Ca_ER_CICR).
   Released cytosolic Ca2+ (ca_cicr_CICR) boosts the effcai integrator;
   no direct gamma_p boost (CICR acts entirely through effcai).
   The Li-Rinzel inactivation gate h_CICR is an additional state variable.
   All intermediate fluxes and gate values are exposed as RANGE variables
   for diagnostic recording. When enable_CICR=0, the model behaves
   identically to the original GluSynapse.
...
NEURON {
    ...
    :[CICR+] ER store dynamics (Li-Rinzel IP3R)
    RANGE enable_CICR
    GLOBAL delta_IP3_CICR, tau_IP3_CICR
    GLOBAL d1_CICR, d2_CICR, d3_CICR, d5_CICR, a2_CICR
    GLOBAL V_CICR_CICR, V_SERCA_CICR, K_SERCA_CICR, V_leak_CICR
    GLOBAL gamma_ER_CICR, tau_extrusion_CICR, Ca_ER_ss_CICR
    RANGE IP3_CICR, Ca_ER_CICR, h_CICR, ca_cicr_CICR
    RANGE ca_e_CICR, J_cicr_CICR, J_serca_CICR, J_leak_CICR
    RANGE Nves_CICR
    :[CICR+] diagnostic RANGE variables for recording
    RANGE m_inf_CICR, n_inf_CICR, Popen_CICR : Li-Rinzel IP3R gate values
    RANGE S_CICR                             : SERCA Hill activation
    RANGE gamma_p_eff_CICR                   : effective gamma_p (kept for compatibility)
    : Misc
...
PARAMETER {
    ...
    tau_ind_GB      = 70        (s)
    tau_exp_GB      = 100       (s)
    tau_effca_GB    = 129.1823           (ms) : fitted (cmaes 2026-03-01)
    gamma_d_GB      = 249.9229           (1)  : fitted (cmaes 2026-03-01)
    gamma_p_GB      = 191.8133           (1)  : fitted (cmaes 2026-03-01)
    theta_d_GB      = 0.006     (us/liter)
    theta_p_GB      = 0.001     (us/liter)
    rho0_GB         = 0         (1)
    :[CICR+] ER store dynamics (Li-Rinzel IP3R) - fitted cmaes 2026-03-01
    :         enable_CICR=0 (default) gives exact original GluSynapse behavior
    enable_CICR       = 1         : 0=off (original GB), 1=on (Li-Rinzel CICR)
    : IP3 dynamics
    delta_IP3_CICR    = 7.1016    : IP3 bolus per vesicle release
    tau_IP3_CICR      = 553.6846  (ms) : IP3 decay time constant
    : Li-Rinzel IP3R gating constants (mM)
    d1_CICR           = 0.7115    : IP3 dissociation constant (IP3 activation)
    d2_CICR           = 0.5164    : Ca inactivation dissociation constant
    d3_CICR           = 0.6019    : IP3 offset in h gate expression
    d5_CICR           = 0.0533    : Ca activation dissociation constant
    a2_CICR           = 1.8171    : inactivation rate constant (/mM/ms)
    : Flux magnitudes
    V_CICR_CICR       = 6.7840    : IP3R max conductance (/ms)
    V_SERCA_CICR      = 2.3493    : SERCA max pump rate (mM/ms)
    K_SERCA_CICR      = 0.8934    : SERCA half-activation (mM)
    V_leak_CICR       = 0.0016    : passive ER leak conductance (/ms)
    : Compartment / scaling
    gamma_ER_CICR     = 0.1255    : ER/cytosol volume ratio
    tau_extrusion_CICR = 163.7724 (ms) : cytosolic CICR Ca clearance
    Ca_ER_ss_CICR     = 100.0     : initial ER Ca2+ (same units as Python model)
    : Misc
    synapseID       = 0
    verbose         = 0
...
ASSIGNED {
    ...
    dep_GB          (1)
    pot_GB          (1)
    :[CICR+] ER dynamics intermediates (Li-Rinzel)
    ca_e_CICR       (mM)        : excess cytosolic Ca (above rest)
    J_cicr_CICR                 : IP3R release flux
    J_serca_CICR                : SERCA uptake flux
    J_leak_CICR                 : passive ER leak flux
    S_CICR                      : SERCA Hill activation
    m_inf_CICR                  : Li-Rinzel IP3 activation gate
    n_inf_CICR                  : Li-Rinzel Ca activation gate
    Popen_CICR                  : Li-Rinzel open probability
    gamma_p_eff_CICR            : effective gamma_p (= gamma_p_GB, no boost)
    Nves_CICR       (1)
    : Misc
    v               (mV)
    vsyn            (mV)
...
STATE {
    ...
    :[CICR+] ER store dynamics (Li-Rinzel IP3R)
    IP3_CICR                 <1e-9>  : cytosolic IP3
    Ca_ER_CICR                        : ER Ca2+ concentration
    h_CICR          (1)      <1e-9>  : Li-Rinzel IP3R inactivation gate
...
INITIAL{
    ...
    h_CICR           = 0.8          : Li-Rinzel: mostly open at rest
    ca_cicr_CICR     = 0
    ca_e_CICR        = 0
    J_cicr_CICR      = 0
    J_serca_CICR     = 0
...
BREAKPOINT {
    ...
    if (enable_CICR > 0) {
        if (IP3_CICR < 0)       { IP3_CICR = 0 }
        if (Ca_ER_CICR < 0)     { Ca_ER_CICR = 0 }
        if (h_CICR < 0)         { h_CICR = 0 }
        if (h_CICR > 1)         { h_CICR = 1 }
...
DERIVATIVE state {
    LOCAL minf_VDCC, hinf_VDCC, ce, ca_i, mloc, nloc, Q2loc, hinfloc, tauhloc, grad, Popenl, Jcicr, Jserca, Jleak, Sloc
    : AMPA Receptor
    ...
    :[CICR+] ER store dynamics — Li-Rinzel IP3R formulation
    ce = cai_CR - min_ca_CR
    if (ce < 0) { ce = 0 }
    ca_e_CICR = ce

    : Total cytosolic Ca seen by IP3R (external + CICR-released)
    ca_i = ce + ca_cicr_CICR

    : Li-Rinzel gating: m (IP3 activation), n (Ca activation), h (Ca inactivation)
    mloc   = IP3_CICR / (IP3_CICR + d1_CICR + 1e-12)
    nloc   = ca_i / (ca_i + d5_CICR + 1e-12)
    Q2loc  = d2_CICR * (IP3_CICR + d1_CICR) / (IP3_CICR + d3_CICR + 1e-12)
    hinfloc = Q2loc / (Q2loc + ca_i + 1e-12)
    tauhloc = 1.0 / (a2_CICR * (Q2loc + ca_i + 1e-12))
    Popenl = mloc*mloc*mloc * nloc*nloc*nloc * h_CICR*h_CICR*h_CICR

    : Expose gating diagnostics
    m_inf_CICR  = mloc
    n_inf_CICR  = nloc
    Popen_CICR  = Popenl

    : SERCA Hill n=2 activation
    Sloc = ca_i*ca_i / (K_SERCA_CICR*K_SERCA_CICR + ca_i*ca_i + 1e-30)
    S_CICR = Sloc
    ...
    : IP3 dynamics — pure decay (bolus added in NET_RECEIVE)
    IP3_CICR'    = enable_CICR * (-IP3_CICR / tau_IP3_CICR)

    : ER Ca2+: SERCA fills, IP3R+leak empties
    Ca_ER_CICR'  = enable_CICR * (Jserca - Jcicr - Jleak) / gamma_ER_CICR

    : Li-Rinzel h inactivation gate
    h_CICR'      = enable_CICR * (hinfloc - h_CICR) / tauhloc

    : Cytosolic CICR Ca pool: released - re-sequestered - extruded
    ca_cicr_CICR' = enable_CICR * (Jcicr + Jleak - Jserca - ca_cicr_CICR / tau_extrusion_CICR)

    : gamma_p_eff — no direct CICR boost; CICR acts via effcai only
    gamma_p_eff_CICR = gamma_p_GB

    : Long-term synaptic plasticity — effcai boosted by CICR Ca
    effcai_GB'   = - effcai_GB/tau_effca_GB + (ce + enable_CICR * ca_cicr_CICR)
