#!/usr/bin/env python3
"""
Minimal CICR Priming Model with P-Modulated Depression (JAX).
...
Design Principles:
  ...
  5. P-modulated depression: dep_scale = max(0, 1 - P/P_thresh).
     Low P (no CICR) → dep_scale ≈ 1 → strong depression → LTD.
     High P (CICR fires) → dep_scale ≈ 0 → no depression + CICR boost → LTP.
     This directly links priming level to plasticity direction.
...
def _debug_sim(cai_1syn, t, cp, cq, is_apical, dp):
    """Numpy debug simulation — CICR Priming with P-Modulated Depression."""
    ...
    for i in range(n - 1):
        ...
        # ── 6. Plasticity (GB) with P-modulated depression ──
        # dep_scale: 1.0 when P=0 (full depression), 0.0 when P≥P_thresh
        dep_scale = max(0.0, 1.0 - P_new / max(P_thresh, 1e-30))
        pot = 1.0 if effcai > theta_p else 0.0
        dep = 1.0 if effcai > theta_d else 0.0
        drho = (-rho * (1 - rho) * (0.5 - rho)
                + gamma_p * (1 - rho) * pot
                - gamma_d * dep_scale * rho * dep) / 70000.0
...
class CICRPrimingModel(CICRModel):
    DESCRIPTION = "CICR Priming + P-Modulated Depression"
