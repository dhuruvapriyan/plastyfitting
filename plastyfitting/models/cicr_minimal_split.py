#!/usr/bin/env python3
"""
Minimal CICR Priming Model with Split CICR (JAX).
...
Design Principles:
  ...
  4. Split CICR: ca_cicr boosts ONLY potentiation, not depression.
       effcai = integrate(ca_ext)           → drives dep (no CICR contamination)
       effcai_pot = effcai + ca_cicr        → drives pot (CICR-boosted)
     -10ms (no CICR): effcai ~0.09 > θ_d → dep=1, effcai_pot ~0.09 < θ_p → LTD
     +10ms (CICR):    effcai ~0.10 > θ_d → dep=1, effcai_pot ~0.9  > θ_p → LTP
...
def _debug_sim(cai_1syn, t, cp, cq, is_apical, dp):
    """Numpy debug simulation — CICR Priming with Split CICR."""
    ...
    for i in range(n - 1):
        ...
        # ── 5. Effective calcium integrator (ca_ext ONLY — no ca_cicr) ──
        d_e = np.exp(-dt / tau_eff)
        effcai = effcai * d_e + ca_ext * tau_eff * (1.0 - d_e)

        # ── 6. Plasticity (GB) — Split CICR: ca_cicr boosts pot only ──
        effcai_pot = effcai + ca_cicr   # CICR boosts potentiation comparison
        pot = 1.0 if effcai_pot > theta_p else 0.0    # needs CICR to cross θ_p
        dep = 1.0 if effcai > theta_d else 0.0        # raw effcai, no CICR
...
class CICRPrimingModel(CICRModel):
    DESCRIPTION = "CICR Priming + Split CICR (pot-only boost)"
    ...
    FIT_PARAMS = [
        ...
        ("a00", 1.0, 3.0), ("a01", 1.0, 3.0),       # θ_d basal
        ("a10", 3.0, 15.0), ("a11", 3.0, 15.0),     # θ_p basal — θ_p > θ_d always
        ("a20", 1.0, 3.0), ("a21", 1.0, 3.0),       # θ_d apical
        ("a30", 3.0, 15.0), ("a31", 3.0, 15.0),     # θ_p apical
        # --- CICR Priming Parameters (5) ---
        ("delta_P", 0.01, 1.0),           # P charging rate — slow priming
        ("tau_P", 60000.0, 300000.0),     # Priming decay time constant (ms)
        ("alpha_thresh", 0.5, 3.0),       # CICR trigger: P > alpha_thresh * θ_p
        ("A_cicr", 0.0005, 0.005),        # CICR release amplitude (mM)
        ("tau_cicr", 100.0, 2000.0),      # CICR release decay time (ms)
    ]
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            def scan_step(carry, inputs):
                ...
                # ── 5. Effective calcium integrator (ca_ext ONLY) ──
                decay_eff = jnp.exp(-dt / tau_effca)
                effcai_new = jnp.where(
                    dt > 0,
                    effcai * decay_eff + ca_ext * tau_effca * (1.0 - decay_eff),
                    effcai,
                )

                # ── 6. Plasticity (GB) — Split CICR: ca_cicr boosts pot only ──
                effcai_pot = effcai_new + ca_cicr_new
                pot = jnp.where(effcai_pot > theta_p, 1.0, 0.0)
