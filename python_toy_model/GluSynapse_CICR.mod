...
/**
 * @file GluSynapse.mod
 * @brief Probabilistic synapse featuring long-term plasticity and Junctional CICR
 * @author king, chindemi, rossert, extended for CICR
 * @date 2026-02-21
 * @version 2.0.0
 * 
...
NEURON {
    ...
    : Junctional CICR (RyR/IP3R Synergy)
    GLOBAL tau_prime_CICR, k_prime_CICR, K_prime_limit_CICR
    GLOBAL K_RyR_CICR, n_RyR_CICR, Vmax_CICR, tau_rel_CICR
    RANGE prime_CICR, ca_cicr
...
PARAMETER {
    ...
    : Junctional CICR (RyR/IP3R Synergy) - Optimized Parameters
    tau_prime_CICR      = 1271.4256 (ms)
    k_prime_CICR        = 27269.7497 (1)
    K_prime_limit_CICR  = 0.0008    (mM)
    K_RyR_CICR          = 0.0037    (mM)
    n_RyR_CICR          = 3.4732    (1)
    Vmax_CICR           = 0.0010    (mM/ms)
    tau_rel_CICR        = 192.5917  (ms)
...
INITIAL{
    ...
    B_NMDA      = 0
    : Stochastic Tsodyks-Markram Multi-Vesicular Release
    if (uncaging_mode) { Use_GB = 1 } else { Use_GB = Use }
    : Postsynaptic Ca2+ dynamics
    cai_CR      = min_ca_CR
    prime_CICR  = 0
    ca_cicr     = 0
...
DERIVATIVE state {
    LOCAL minf_VDCC, hinf_VDCC, cai_evoked, cai_clip, rate_load, trigger, J_rate
    ...
    : CICR
    cai_evoked = cai_CR - min_ca_CR
    if (cai_evoked < 0.0) { cai_evoked = 0.0 }
    
    : CICR - Soft-Clipped ER Priming
    cai_clip = K_prime_limit_CICR * tanh(cai_evoked / K_prime_limit_CICR)
    rate_load = k_prime_CICR * cai_clip
    prime_CICR' = rate_load * (1.0 - prime_CICR) - prime_CICR / tau_prime_CICR

    : CICR - RyR/Junctional Trigger
    if (cai_evoked > 0.0) {
        trigger = pow(cai_evoked, n_RyR_CICR) / (pow(K_RyR_CICR, n_RyR_CICR) + pow(cai_evoked, n_RyR_CICR) + 1e-12)
    } else {
        trigger = 0.0
    }
    
    : CICR - CICR Release
    J_rate = Vmax_CICR * prime_CICR * trigger
    ca_cicr' = J_rate - ca_cicr / tau_rel_CICR

    : Long-term synaptic plasticity
    effcai_GB'   = - effcai_GB/tau_effca_GB + (cai_evoked + ca_cicr)
