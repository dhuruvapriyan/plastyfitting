...
NEURON {
    ...
    GLOBAL tau_r_NMDA_Ca, tau_d_NMDA_Ca
    RANGE gmax_NMDA, g_NMDA, g_NMDA_Ca, i_AMPA, i_NMDA
    RANGE mg
    ...
    RANGE ica_NMDA
    : Spine
    RANGE volume_CR, cai_CR, cai_NMDA_CR, cai_VDCC_CR, effcai_GB
    : VDCC (R-type)
    GLOBAL ljp_VDCC, vhm_VDCC, km_VDCC, mtau_VDCC, vhh_VDCC, kh_VDCC, htau_VDCC, gca_bar_VDCC
    RANGE ica_VDCC
    : Postsynaptic Ca2+ dynamics
    GLOBAL gamma_ca_CR, tau_ca_CR, tau_ca_NMDA_CR, tau_ca_VDCC_CR, min_ca_CR, cao_CR
    : Long-term synaptic plasticity
    GLOBAL rho_star_GB, tau_ind_GB, tau_exp_GB, tau_effca_GB
    GLOBAL gamma_d_GB, gamma_p_GB
    RANGE theta_d_GB, theta_p_GB, rho0_GB, dep_GB, pot_GB, w_NMDA_GB, w_VDCC_GB
    : Misc
...
PARAMETER {
    ...
    tau_r_NMDA_Ca   = 0.29      (ms)        : Tau rise for the plasticity-driving NMDAR Ca trace
    tau_d_NMDA_Ca   = 20        (ms)        : Tau decay for the plasticity-driving NMDAR Ca trace
    ...
    tau_ca_NMDA_CR  = 12        (ms)        : Rate of removal of NMDAR-driven calcium pool
    tau_ca_VDCC_CR  = 12        (ms)        : Rate of removal of VDCC-driven calcium pool
    ...
    rho0_GB         = 0         (1)
    w_NMDA_GB       = 1.0       (1)         : Weight of NMDAR calcium into the GB integrator
    w_VDCC_GB       = 0.3       (1)         : Weight of VDCC calcium into the GB integrator
    : Misc
    synapseID       = 0
    verbose         = 0
...
ASSIGNED {
    ...
    g_NMDA_Ca       (uS)    : Fast NMDAR conductance used only for plasticity-driving calcium
    : Stochastic Tsodyks-Markram Multi-Vesicular Release
    ...
    cai_CR          (mM)    : Total postsynaptic calcium for reporting and Eca calculation
    : Long-term synaptic plasticity
    dep_GB          (1)
    pot_GB          (1)
...
INITIAL{
    ...
    : Postsynaptic Ca2+ dynamics
    cai_NMDA_CR = min_ca_CR
    cai_VDCC_CR = min_ca_CR
    cai_CR      = min_ca_CR
    : Long-term synaptic plasticity
    rho_GB      = rho0_GB
    effcai_GB   = 0
...
BREAKPOINT {
    ...
    g_NMDA_Ca = (1e-3)*gmax_NMDA*mggate*(B_NMDA_Ca - A_NMDA_Ca)
    ica_NMDA = Pf_NMDA*g_NMDA_Ca*(v-40.0)
    : Total postsynaptic Ca2+ for reporting and Eca calculation
    cai_CR = cai_NMDA_CR + cai_VDCC_CR - min_ca_CR
...
DERIVATIVE state {
    ...
    A_NMDA_Ca'   = - A_NMDA_Ca/tau_r_NMDA_Ca
    B_NMDA_Ca'   = - B_NMDA_Ca/tau_d_NMDA_Ca
    : Stochastic Tsodyks-Markram Multi-Vesicular Release
    if (uncaging_mode) {
        Use_GB'  = 0
    } else {
    ...
    : Long-term synaptic plasticity
    effcai_GB'   = - effcai_GB/tau_effca_GB
                   + w_NMDA_GB*(cai_NMDA_CR - min_ca_CR)
                   + w_VDCC_GB*(cai_VDCC_CR - min_ca_CR)
    rho_GB'      = ( - rho_GB*(1 - rho_GB)*(rho_star_GB - rho_GB)
                     + pot_GB*gamma_p_GB*(1 - rho_GB)
                     - dep_GB*gamma_d_GB*rho_GB ) / ((1e3)*tau_ind_GB)
    : Postsynaptic Ca2+ dynamics
    cai_NMDA_CR' = - (1e-9)*ica_NMDA*gamma_ca_CR/((1e-15)*volume_CR*2*FARADAY)
                   - (cai_NMDA_CR - min_ca_CR)/tau_ca_NMDA_CR
    cai_VDCC_CR' = - (1e-9)*ica_VDCC*gamma_ca_CR/((1e-15)*volume_CR*2*FARADAY)
                   - (cai_VDCC_CR - min_ca_CR)/tau_ca_VDCC_CR
}
    ...
    if(flag == 0) {
        if(weight <= 0){
            ...
            if(verbose > 0){ printf("Inactive synapse, weight = %g\n", weight) }
                ...
                : Update fast NMDA calcium variables
                tp = (tau_r_NMDA_Ca*tau_d_NMDA_Ca)/(tau_d_NMDA_Ca-tau_r_NMDA_Ca)*log(tau_d_NMDA_Ca/tau_r_NMDA_Ca)  : Time to peak
                factor = 1 / (-exp(-tp/tau_r_NMDA_Ca)+exp(-tp/tau_d_NMDA_Ca))  : Normalization factor
                A_NMDA_Ca = A_NMDA_Ca + released/Nrrp*factor
                B_NMDA_Ca = B_NMDA_Ca + released/Nrrp*factor
                : Update tsyn
