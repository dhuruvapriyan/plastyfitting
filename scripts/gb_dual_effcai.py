#!/usr/bin/env python3
"""
GB model with dual NMDA/VDCC effcai integrators.

Key innovation over gb_only / gb_only_weighted:
  - Two parallel effcai integrators run INSIDE the scan step, driven by
    cai_NMDA_CR and cai_VDCC_CR separately at every timestep.
  - Depression (LTD): either NMDA effcai > theta_d_nmda  OR  VDCC effcai > theta_d_vdcc
  - Potentiation (LTP): NMDA effcai > theta_p_nmda  AND  VDCC effcai > theta_p_vdcc

The AND gate for LTP is a coincidence detector: it fires only when both
NMDA-driven and VDCC-driven calcium are simultaneously elevated.

  For +10ms (pre→post): EPSP arrives first → Mg²⁺ unblock when back-AP
    arrives → large NMDA effcai + moderate VDCC effcai → AND fires → LTP.

  For -10ms (post→pre): back-AP (VDCC) fires first, NMDA sees no Mg²⁺
    unblock (membrane repolarised) → large VDCC effcai only → OR fires
    → LTD; AND cannot fire → no LTP.

Parameters (17 total, no alpha/beta — uses raw NMDA and VDCC traces):
  gamma_d, gamma_p                      — plasticity rates
  d_n0, d_n1, p_n0, p_n1               — NMDA threshold coefficients (basal)
  d_v0, d_v1, p_v0, p_v1               — VDCC threshold coefficients (basal)
  d_n0a, d_n1a, p_n0a, p_n1a           — NMDA threshold coefficients (apical)
  d_v0a, d_v1a, p_v0a, p_v1a           — VDCC threshold coefficients (apical)
  tau_eff                               — shared effcai time constant

Usage
-----
    python gb_dual_effcai.py --method de --max-iter 2000 --dt-step 10 \\
...
class GBDualEffcaiModel(WeightedCICRModel):
    """GB model with separate NMDA/VDCC effcai integrators and AND/OR gates."""

    DESCRIPTION = "GB Dual NMDA/VDCC EffCai (coincidence-detection LTP)"
    NEEDS_THRESHOLD_TRACES = True
    NEEDS_RAW_CAI_TRACE    = False   # we only use cai_nmda / cai_vdcc
    NEEDS_BASE_THRESHOLD_TRACES = True
    NEEDS_CPRE_CPOST        = False   # c_pre/c_post computed from NMDA/VDCC peaks

    # ------------------------------------------------------------------ #
    # Parameter layout (index, name, lo, hi)
    # [0]  gamma_d      [1]  gamma_p
    # [2]  d_n0  [3]  d_n1  [4]  p_n0  [5]  p_n1   NMDA basal
    # [6]  d_v0  [7]  d_v1  [8]  p_v0  [9]  p_v1   VDCC basal
    # [10] d_n0a [11] d_n1a [12] p_n0a [13] p_n1a  NMDA apical
    # [14] d_v0a [15] d_v1a [16] p_v0a [17] p_v1a  VDCC apical
    # [18] tau_eff
    # ------------------------------------------------------------------ #
    FIT_PARAMS = [
        ("gamma_d",  50.0,  500.0),
        ("gamma_p",  50.0,  500.0),
        # NMDA thresholds — basal
        ("d_n0",  1,  30.0), ("d_n1",  1,  30.0),
        ("p_n0",  1,  30.0), ("p_n1",  1,  30.0),
        # VDCC thresholds — basal
        ("d_v0",  1,  30.0), ("d_v1",  1,  30.0),
        ("p_v0",  1,  30.0), ("p_v1",  1,  30.0),
        # NMDA thresholds — apical
        ("d_n0a", 1,  30.0), ("d_n1a", 1,  30.0),
        ("p_n0a", 1,  30.0), ("p_n1a", 1,  30.0),
        # VDCC thresholds — apical
        ("d_v0a", 1,  30.0), ("d_v1a", 1,  30.0),
        ("p_v0a", 1,  30.0), ("p_v1a", 1,  30.0),
    ...
    DEFAULT_PARAMS = {
        ...
        "d_n0":  2.0, "d_n1":  2.0, "p_n0":  4.0, "p_n1":  4.0,
        "d_v0":  2.0, "d_v1":  2.0, "p_v0":  4.0, "p_v1":  4.0,
        "d_n0a": 2.0, "d_n1a": 2.0, "p_n0a": 4.0, "p_n1a": 4.0,
        "d_v0a": 2.0, "d_v1a": 2.0, "p_v0a": 4.0, "p_v1a": 4.0,
        "tau_eff": 100.0,
    }
    ...
    def unpack_params(self, x):
        gb = {
            ...
            "d_n0":  x[2],  "d_n1":  x[3],  "p_n0":  x[4],  "p_n1":  x[5],
            "d_v0":  x[6],  "d_v1":  x[7],  "p_v0":  x[8],  "p_v1":  x[9],
            "d_n0a": x[10], "d_n1a": x[11], "p_n0a": x[12], "p_n1a": x[13],
            "d_v0a": x[14], "d_v1a": x[15], "p_v0a": x[16], "p_v1a": x[17],
        }
        aux = {"tau_eff": x[18]}
        return gb, aux
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            # syn_params: (_, _, is_apical,
            #              _, cai_pre_nmda, cai_pre_vdcc, t_pre,
            #              _, cai_post_nmda, cai_post_vdcc, t_post)
            (_, _, is_apical,
             _, cai_pre_nmda, cai_pre_vdcc, t_pre,
            ...
            # Peak effcai from single-pulse NMDA and VDCC threshold traces
            ...
            # --- Thresholds: direction-sensitive by construction ---
            # NMDA thresholds (potentiation requires strong NMDA → pre-before-post)
            theta_d_nmda = jnp.where(is_apical,
                gb["d_n0a"] * c_pre_nmda + gb["d_n1a"] * c_post_nmda,
                gb["d_n0"]  * c_pre_nmda + gb["d_n1"]  * c_post_nmda)
            theta_p_nmda = jnp.where(is_apical,
                gb["p_n0a"] * c_pre_nmda + gb["p_n1a"] * c_post_nmda,
                gb["p_n0"]  * c_pre_nmda + gb["p_n1"]  * c_post_nmda)

            # VDCC thresholds
            theta_d_vdcc = jnp.where(is_apical,
                gb["d_v0a"] * c_pre_vdcc + gb["d_v1a"] * c_post_vdcc,
                gb["d_v0"]  * c_pre_vdcc + gb["d_v1"]  * c_post_vdcc)
            theta_p_vdcc = jnp.where(is_apical,
                gb["p_v0a"] * c_pre_vdcc + gb["p_v1a"] * c_post_vdcc,
                gb["p_v0"]  * c_pre_vdcc + gb["p_v1"]  * c_post_vdcc)

            def scan_step(carry, inputs):
                effcai_nmda, effcai_vdcc, rho = carry
                # inputs: (cai_nmda_t, cai_vdcc_t, dt, nves)
                ...
                # LTD:  EITHER channel exceeds its depression threshold
                dep = jnp.where(
                    (effcai_vdcc_new > theta_d_vdcc),
                    1.0, 0.0
                )
                # dep = jnp.where(
                #     (effcai_nmda_new > theta_d_nmda),
                #     1.0, 0.0
                # )
                # LTP:  BOTH channels must exceed their potentiation threshold
                #       (coincidence detection — direction-sensitive)
                pot = jnp.where(
                    (effcai_vdcc_new > theta_p_vdcc),
                    1.0, 0.0
                )
    ...
    # ------------------------------------------------------------------
    # Override setup_jax: pass (cai_nmda, cai_vdcc, dt, nves) to scan
    # instead of the usual combined cai_weighted.
    # Everything else (vmappair, objectives) is unchanged from
    # WeightedCICRModel.
    # ------------------------------------------------------------------
    def setup_jax(self, protocol_data, targets,
        ...
        raw_collated = {
            p: collate_protocol_to_jax(
                ...
                dt_step=dt_step,
                interp_dt=interp_dt,
                include_raw_cai=False,           # don't need combined cai_CR
        ...
        # sim_synapse: key change — scan over (cai_nmda, cai_vdcc, dt, nves)
        def sim_synapse(cai_nmda_trace, cai_vdcc_trace, t_trace, nves_trace,
            ...
            # init: (effcai_nmda=0, effcai_vdcc=0, rho=rho0)
            init = init_fn(cai_nmda_trace[0], rho0)

            syn_params = (
                0.0, 0.0, is_apical,                        # c_pre, c_post (unused), is_apical
            ...
            # Scan over both channels simultaneously
        ...
        # vmap over synapses (axis 0); t, nves are per-pair (None)
        vmapsyn = jax.vmap(
            sim_synapse,
            in_axes=(
                0, 0,       # cai_nmda_trace, cai_vdcc_trace  [syn, T]
                None, None, # t_trace, nves_trace              [T]
                0, 0, 0, None, 0,   # is_apical, rho0, sm, bmean, valid
                0, 0, None,  # cai_pre_nmda, cai_pre_vdcc, t_pre  [syn, Tpre] / [Tpre]
                0, 0, None,  # cai_post_nmda, cai_post_vdcc, t_post
                None,        # params
            )
        )
