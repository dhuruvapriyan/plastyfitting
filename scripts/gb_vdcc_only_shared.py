#!/usr/bin/env python3
"""
GB model with a single VDCC effcai integrator — shared basal/apical params.

Same as gb_vdcc_only.py but collapses the basal/apical split into one set
of threshold coefficients shared across all synapses.

Parameters (7 total):
  gamma_d, gamma_p
  d_v0, d_v1   — LTD VDCC threshold (pre-weight, post-weight)
  p_v0, p_v1   — LTP VDCC threshold (pre-weight, post-weight)
  tau_eff
...
Usage
-----
    python gb_vdcc_only_shared.py --method de --max-iter 2000 --dt-step 10 \\
...
class GBVdccOnlySharedModel(WeightedCICRModel):
    """Single VDCC effcai integrator; shared thresholds for basal and apical."""

    DESCRIPTION = "GB VDCC-Only EffCai SHARED (7 params, no apical split)"
    ...
    # fmt: off
    FIT_PARAMS = [
        ("gamma_d_GB_GluSynapse",  50.0, 500.0),
        ("gamma_p_GB_GluSynapse", 150.0, 500.0),
        # VDCC thresholds — shared for all synapses
        # a00/a01 = basal (and apical) depression  pre/post weight
        # a10/a11 = basal (and apical) potentiation pre/post weight
    ...
    DEFAULT_PARAMS = {
        "gamma_d_GB_GluSynapse": 100.0, "gamma_p_GB_GluSynapse": 300.0,
        "a00": 2.0, "a01": 2.0,
        "a10": 4.0, "a11": 4.0,
        "tau_eff": 100.0,
    }
    ...
    def unpack_params(self, x):
        gb = {
            "gamma_d_GB_GluSynapse": x[0], "gamma_p_GB_GluSynapse": x[1],
            # depression thresholds (shared basal+apical)
            "a00": x[2], "a01": x[3],
            # potentiation thresholds (shared basal+apical)
            "a10": x[4], "a11": x[5],
        }
        aux = {"tau_eff": x[6]}
        return gb, aux
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            # Single-pulse peak VDCC effcai
            c_pre_vdcc  = _jax_peak_effcai_zoh(cai_pre,  tau_effca, dt_pre,  _MIN_CA)
            c_post_vdcc = _jax_peak_effcai_zoh(cai_post, tau_effca, dt_post, _MIN_CA)

            # jax.debug.print(
            #     "  [scan_factory] tau_eff={tau:.4f}  c_pre_vdcc={cpre:.6e}  c_post_vdcc={cpost:.6e}",
            #     tau=tau_effca, cpre=c_pre_vdcc, cpost=c_post_vdcc,
            # )

            # Thresholds — shared for all synapses
            # a00/a01: depression pre/post weight  (= basal a00/a01 = apical a20/a21)
            # a10/a11: potentiation pre/post weight (= basal a10/a11 = apical a30/a31)
            theta_d_vdcc = gb["a00"] * c_pre_vdcc + gb["a01"] * c_post_vdcc
            theta_p_vdcc = gb["a10"] * c_pre_vdcc + gb["a11"] * c_post_vdcc
            ...
            def scan_step(carry, inputs):
                ...
                drho = (
                    -rho * (1.0 - rho) * (0.5 - rho)
                    + pot * gb["gamma_p_GB_GluSynapse"] * (1.0 - rho)
                    - dep * gb["gamma_d_GB_GluSynapse"] * rho
                ) / 70000.0
