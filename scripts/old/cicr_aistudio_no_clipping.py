#!/usr/bin/env python3
"""
Primed-Junctional CICR Model V4.3 — Simplified Slow Water Bucket (No Clipping)
...
3. The filling rate is bounded. Narrow bAPs naturally fail to fill the bucket 
   because they are too brief. Wide EPSPs successfully fill the bucket over time.
...
def _apply_primed_junctional_debug(cai_1syn, t, cp, cq, is_apical, dp):
    ...
    for i in range(n - 1):
        ...
        # 2. Simplified Slow Water Bucket
        fill_gate = 1.0 if effcai > theta_d else 0.0
        
        # Raw linear filling (No tanh clipping required, width of spike limits volume naturally)
        rate_load = k_fill * ca_e * fill_gate
...
class PrimedJunctionalCICRModel(CICRModel):
    DESCRIPTION = "Primed-Junctional V4.3 (Simplified Slow Bucket, No Clip)"
    ...
    FIT_PARAMS = [
        ...
        ("a30", 1.0, 10.0),
        ("a31", 1.0, 5.0),
        ("k_fill", 0.0001, 0.5),       # Rate scale adjusted down since input is raw Ca2+
        ...
        ("g_cicr", 10.0, 5000.0),      # Gain multiplier for LTP burst
        ("tau_eff", 50.0, 500.0),      # Leaky integrator tau
    ]

    DEFAULT_PARAMS = {
        "gamma_d_GB_GluSynapse": 150.0, "gamma_p_GB_GluSynapse": 200.0,
        "a00": 1.5, "a01": 1.5, "a10": 2.0, "a11": 2.0,
        "a20": 1.5, "a21": 1.5, "a30": 2.0, "a31": 2.0,
        "k_fill": 0.01, "tau_leak": 120000.0, 
        "K_trig": 0.0025, "n_trig": 6.0,
        "Vmax_CICR": 0.1, "tau_CICR": 250.0,
        "g_cicr": 500.0, "tau_eff": 200.0,
    }
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            def scan_step(carry, inputs):
                ...
                # 2. Simplified Slow Water Bucket (Linear Integration)
                fill_gate = jnp.where(effcai_new > theta_d, 1.0, 0.0)
                
                rate_load = cicr['k_fill'] * ca_e * fill_gate
