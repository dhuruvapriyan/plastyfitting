...
K_T is location-aware (scales with dendritic position):
    peak_pre  = max(cai_pre  - cai_rest)   # single pre-spike peak for this synapse
    peak_post = max(cai_post - cai_rest)   # single post-spike peak for this synapse
    K_T = k_T * (peak_pre + peak_post)

    k_T in (0.3, 0.9) is the single free fraction parameter.
    Single pre OR post spike  →  Ca_e ≈ peak_pre or peak_post  <  K_T  →  RyR silent
    Coincident pre+post       →  Ca_e ≈ peak_pre + peak_post   >  K_T  →  RyR fires

    This mirrors the theta_d/theta_p = a*cpre + b*cpost architecture and
    automatically adapts K_T to distal (large peaks) vs proximal (small peaks)
    synapses, preventing constitutive RyR firing at distal locations.

n_T is fixed at N_T = 10 (effectively a step function; optimizer converged here).

Fixed constants: n_H=4, K_H=0.0004, K_act=0.0003, K_IP3=1.0, K_PLC=0.0005,
P_ss=0.3, tau_RyR=100, cai_rest=70e-6, n_T=10
"""
...
CAI_REST = 70e-6
TRACE_TAU_REF = 200.0
N_T = 10.0          # fixed; optimizer converged to ~10 (step-function regime)
...
def _v5_debug(cai_1syn, t, cp, cq, is_apical, dp):
    ...
    # --- location-aware K_T ---
    # compute_single (plot_cicr_diagnostic.py) should inject dp["cai_pre"] and
    # dp["cai_post"] for the specific synapse (see patch note below).
    # Graceful fallbacks are provided for legacy callers.
    if "cai_pre" in dp and "cai_post" in dp:
        peak_pre  = float(max(0.0, np.max(dp["cai_pre"])  - CAI_REST))
        peak_post = float(max(0.0, np.max(dp["cai_post"]) - CAI_REST))
        K_T = k_T * (peak_pre + peak_post)
    elif "K_T" in dp:                          # legacy param dict fallback
        K_T = float(dp["K_T"])
    else:
        K_T = k_T * 0.003                      # ~proximal synapse typical sum
    ...
    return {
        ...
        "effcai":    effcai_out,
        "rho":       rho_out,
        "K_T_used":  K_T,          # exposed for diagnostics
    }
...
class ERDynamicsCICRModel(CICRModel):
    ...
    # 19 free parameters (down from 20: K_T absorbed into k_T; n_T fixed)
    FIT_PARAMS = [
        # --- Graupner-Brunel plasticity rule ---
        ("gamma_d_GB_GluSynapse", 50.0,   250.0),
        ("gamma_p_GB_GluSynapse", 50.0,   300.0),
        # basal theta coefficients
        ("a00", 1.0,  2.5),
        ("a01", 1.0,  5.0),
        ("a10", 1.0,  5.0),
        ("a11", 1.0,  5.0),
        # apical theta coefficients
        ("a20", 1.0,  2.5),
        ("a21", 1.0,  5.0),
        ("a30", 1.0, 10.0),
        ("a31", 1.0,  5.0),
        ...
        ("k_leak",    1e-6,  0.001),
        ("V_RyR",     0.0001, 0.05),
        # k_T: fraction of (peak_pre + peak_post) that sets the RyR threshold
        # replaces the absolute K_T; single pre/post spike → Ca_e < K_T; coincidence → Ca_e > K_T
        ("k_T",   0.3,  0.9),
        ("V_IP3R", 0.0001, 0.01),
        ("g_RyR",  10.0, 50000.0),
        ("tau_eff", 50.0,  500.0),
    ...
    DEFAULT_PARAMS = {
        ...
        "a00": 1.0003490121729528, "a01": 1.0877438002708641,
        "a10": 1.3917207001380245, "a11": 4.129505808304705,
        "a20": 2.3079274077684175, "a21": 1.9619384872398133,
        "a30": 4.456560780131431,  "a31": 4.80485148600644,
        "delta_IP3": 2.3564846028724626,
        "tau_IP3":   4198.8339980611045,
        "phi_serca": 0.00010926586342370828,
        "k_leak":    0.00038983484712417705,
        "V_RyR":     0.0008831061403601626,
        # k_T ≈ K_T_old / (typical peak_pre + peak_post)
        # K_T_old ≈ 0.0017 mM; typical sum ≈ 0.003 mM → k_T ≈ 0.57
        "k_T":   0.57,
        "V_IP3R": 0.009883023779226525,
        "g_RyR":  14060.428640146583,
        "tau_eff": 306.6936809019515,
    }
    ...
    def unpack_params(self, x):
        ...
        cicr = {
            ...
            'V_RyR':     x[14], 'k_T':     x[15],
            'V_IP3R':    x[16], 'g_RyR':   x[17], 'tau_eff': x[18],
        }
        return gb, cicr
    ...
    def get_step_factory(self):
        def scan_factory(params, syn_params):
            ...
            # --- location-aware K_T (per-synapse, JAX-compatible) ---
            # peak_pre / peak_post = Ca_e peak from a single pre / post spike
            peak_pre  = jnp.max(jnp.maximum(0.0, cai_pre  - CAI_REST))
            peak_post = jnp.max(jnp.maximum(0.0, cai_post - CAI_REST))
            K_T = cicr['k_T'] * (peak_pre + peak_post)

            # --- theta_d / theta_p from effcai of single-spike traces ---
            ...
            # fixed / module-level constants captured in closure
            n_H_c     = N_H
            K_H_c     = K_H
            n_T_c     = N_T      # fixed step-function exponent
            K_act_c   = K_ACT
            K_IP3_c   = K_IP3
            K_PLC_c   = K_PLC
            P_ss_c    = P_SS
            ...
            def scan_step(carry, inputs):
                ...
                # RyR Hill: K_T is location-aware (computed above, captured by closure)
                ca_e_nT = ca_e ** n_T_c
                K_T_nT  = K_T  ** n_T_c
